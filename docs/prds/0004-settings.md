...
writes no chain; a valid one folds back to exactly those settings).
...
is a new set of keys, not a schema break.
...
wrong file finds out).

A joining member carries no settings at all; it gets
