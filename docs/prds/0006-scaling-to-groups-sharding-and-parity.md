...
4. A ledger (or a range of one) can be *owned* by one group — that group's
   leader sequences it, that group's members replicate it in full — and
   other groups route to it rather than copy it.
5. A ledger can optionally be stored with parity across groups, so k of m
...
- **No global total order across groups.** Each owned ledger has its chain;
  there is no cross-group chain in v1 of the overlay. A consumer that needs
  one puts its data in one ledger.
- **No automatic rebalancing of ledgers between groups in the first
  overlay release.** Ownership moves by an operator-visible settings change.
- **No cross-group transactions.** Append-only, one ledger, one group.
...
| 2 | 32–~1,000 | several groups; each a tier-1 group | group membership, ledger ownership, routing | this PRD, phase 1–2 |
| 3 | ~1,000–100,000 | groups of groups | recursive membership; leader-star or gossip inside large groups | this PRD, phase 3; OQ 25 |
| parity | any, with tier ≥ 2 | — | k-of-m storage across groups for selected ledgers | this PRD, phase 4 |
...
ledger: group membership, ledger ownership, and federation-level settings.
...
The unit of ownership is a **ledger** (the chain-per-ledger choice in
[OQ 7](../open-questions.md) is what makes this clean: a ledger is a
self-contained chain that can live in one group). The federation's control
ledger maps `ledger id → owning group`. Inside the owning group, nothing is
different from tier 1: that group's leader sequences, its members hold the
full chain. Other groups hold *no copy* of that ledger — they hold the map.

A consumer anywhere appends to ledger L by handing the entry to its local
...
honoured by the owning group). Reads of a ledger the local group does not
...
member, never sequencing) — the per-ledger setting `replicate.followers`
names which groups keep one.

A ledger that is too hot for one group is split by **range**: `ledger id +
...
Ownership moves by a federation `settings` entry (`ledger.owner = group`);
...
cold or large ledgers the operator's "break the ledger down and have data
parity" is erasure coding across groups: segments of a ledger's chain are
...
Parity is a per-ledger setting (`storage.parity = {k, m}`) applied to
...
| Now | Why | Where |
|---|---|---|
| Chain per ledger, not per cluster | a ledger is the unit of ownership and of parity; a cluster-wide chain could not be split | OQ 7 → resolve as per-ledger |
| Ledger ids are globally unique (128-bit, random or hash-derived), never small integers or per-cluster counters | a ledger must keep its id when its owner changes or when a group joins a federation | PRD 0001 header |
...
| `author_seq` dense per `(author, ledger)` and the author id is the range key | one author's stream is always in one group; range splits by author prefix need no renumbering | PRD 0001, this PRD |
...
| Settings have a scope; add `federation` as a third scope alongside `cluster` and `ledger` *now* so the schema does not need a breaking change | ownership map and federation mode live there | PRD 0004 |
...
| The read/append API takes a ledger id and never assumes the local group owns it | forwarding is an implementation detail, not an API change | PRD 0005 |
...
3. Range split by author prefix; leader-star or gossip inside large groups
   (OQ 25). Trigger: a group at `max_members`, or a ledger too hot for one
   group.
...
| Federation control ledger partitions | it is a cluster: its own mode decides stall or two branches + merge |
...
- [ ] (G4) Two groups, one ledger owned by each; a consumer on either group
  appends to and reads from both; the non-owning group holds no copy.
- [ ] (G5) A ledger stored with `{k=2, m=3}` across three groups reads
