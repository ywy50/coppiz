...
[OQ 54](0007-tier-number-measurements.md); this suite shares its harness where possible
...
- **W5 - join/backfill.** A fresh member joins a cluster holding a 1 GB
  journal; measure time-to-head, MB/s, and peak network. [OQ 54](0007-tier-number-measurements.md)
...
- **W7 - memory and connections at n = 1, 3, 6.** RSS per member, fd count,
  connection count ([OQ 54](0007-tier-number-measurements.md)) sibling). The "library vs
  server" claim (C3) mostly shows up here.
- **W8 - multi-process one-file (SQLite-only row).** SQLite's file-lock
  habit coppiz v1 lacks ([OQ 47](../rfcs/0006-multi-process-one-data-directory.md)). Benchmark it anyway:
...
- **Already in the tree:** [OQ 54](0007-tier-number-measurements.md)'s internal
...
- **The tier numbers changing** ([OQ 54](0007-tier-number-measurements.md)) measurements)
  - the comparison rows stay, the internal reference points move.

## References

- **coppiz records:** [docs/README.md](../README.md),
  [OQ 3, 19, 36, 47, 54](../rfcs/0037-write-ack-layer.md),
