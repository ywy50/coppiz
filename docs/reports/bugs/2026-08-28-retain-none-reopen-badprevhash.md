...
A member that compacted under `retain = none` and restarts fails to open: the fold rebuild (`Node.open` → `foldAll` → `foldJournal` → `store.scan`) hits a chain-continuity refusal and the node cannot start. `refold()` (the [OQ 44](../../prds/0002-ttl-and-staleness.md) heal path) breaks identically. The shipped e2e test exercising `retain = none` (`cluster/node.zig:3118`) never reopens the node, so the gap is uncovered.
