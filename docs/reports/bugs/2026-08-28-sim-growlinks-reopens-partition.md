...
The simulator is the deterministic driver for pure logic ([OQ 27](../../open-questions.md)); its liveness views (`stateOf` → `.lost`), the broadcast gate (`linkOpen`, used by `slotAndBroadcast`), and `heal` all key off `links`. `growLinks` silently heals the world underneath them.
