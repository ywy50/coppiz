...
| **admitter** | the existing member (normally the leader) that writes a newcomer's `join` entry after admission; it decides when, so it may reorder concurrent joins ([OQ 58](open-questions.md#oq-58)), never place anyone before an already-slotted member | PRD 0003, RFC 0002 |
