...
| **admitter** | the existing member (normally the leader) that writes a newcomer's `join` entry after admission; it decides when, so it may reorder concurrent joins ([OQ 58](prds/0003-membership-and-leadership.md)), never place anyone before an already-slotted member | PRD 0003, RFC 0002 |
...
| **OQ n** | the historical ID of an open question from before the register was removed (2026-08-29); the record that settled it names the number - an [RFC](rfcs/), an [ADR](adrs/), a [PRD](prds/), a [research](research/) note, or a [report](reports/README.md) | - |
